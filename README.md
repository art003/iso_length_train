...
| `output\Изометрии_без_API\` | **размеченные** изометрии 1–277 (markup + overlays) |
| `output\тест_1_без_API\` … `тест_3_без_API\` | контрольные тесты с разметкой |
| `output\02_Изометрии_10_листов_без_API\` | эталон 10 листов |
| `output\tests_summary.json` | суммы трёх тестов |
...
Числа из таблицы «Длины отрезков», спецификации и штампа **не считаются** — только размеры на оси трубы.

## Контрольные тесты (после обновления)

```bat
python run_three_tests.py
```

Ожидаемые суммы (мм): тест_1 = **234405**, тест_2 = **260867**, тест_3 = **324203**.
...
- `data\Изометрии.pdf` (большой исходник; копии PDF в `output\` тоже игнорятся)
- OneDrive `desktop.ini`

Оставить: код, `requirements.txt`, `models\iso_clf.pkl`, `data\02_Изометрии_10_листов.pdf`, `data\контрольные_тесты\`, `docs\`, **размеченные** `output\*\markup` и `overlays`, `output\проверка_мастеру\`.
